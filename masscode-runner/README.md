...
> - **环境检测**：启动时检测本地工具及 Draw.io 在线编辑服务（含 LaTeX、Biber、CTeX、OpenSSH、diagrams.net 和浏览器端 AI 绘图配置），此后每 60 秒自动复检，并根据 vault 中实际使用的语言和 `.drawio` 文件计算项目所需环境；网络服务不可用时只给出诊断，不会误加入一键安装命令。顶部「🔍 环境」绿点表示就绪、红点表示缺失。
